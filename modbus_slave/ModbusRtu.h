...
public:
  ...
  int8_t poll_slave(); //!<cyclic poll for slave
...
void Modbus::sendTxBuffer() {
  ...
  while (!(UCSR1A & (1 << TXC1)));
    ...
    default:
      while (!(UCSR0A & (1 << TXC0)));
      //#error "Did not assigne Serial port"
      break;
    }
#endif
...
uint8_t Modbus::GetRReg(uint16_t adrReg,   uint16_t* pReg)
{
	if (adrReg >= SIZE_R_REGS) return ERROR_OF_ADR;
	if (pReg == 0) return ERROR_OF_POINTER;
	
	*pReg = r_regArea[adrReg];
	return 0;
}

uint8_t Modbus::GetRRegs(uint16_t adrReg,  uint16_t* pRegs, uint16_t nRegs)
{
	if ((adrReg  + nRegs) >= SIZE_R_REGS) return ERROR_OF_ADR;
	if (pRegs == 0) return ERROR_OF_POINTER;
...
uint8_t Modbus::GetRWReg(uint16_t adrReg,  uint16_t* pReg)
{
...
uint8_t Modbus::GetRWRegs(uint16_t adrReg, uint16_t* pRegs, uint16_t nRegs)
{
...
uint8_t Modbus::SetRWReg(uint16_t adrReg,  uint16_t pReg)
{
	if (adrReg >= SIZE_RW_REGS) return ERROR_OF_ADR;
	if (pReg == 0) return ERROR_OF_POINTER;
...
uint8_t Modbus::GetRBit(uint16_t adrBit,   uint8_t* pBit)
{
	if (adrBit >= SIZE_R_BITS) return ERROR_OF_ADR;
	if (pBit == 0) return ERROR_OF_POINTER;
	
	if (r_bitArea[adrBit/16] & (1 << (adrBit % 16)))
	{
		*pBit = 1;
	}
	else
...
uint8_t Modbus::GetRwBit(uint16_t adrBit,   uint8_t* pBit)
{
	if (adrBit >= SIZE_RW_BITS) return ERROR_OF_ADR;
	if (pBit == 0) return ERROR_OF_POINTER;
	
	if (rw_bitArea[adrBit/16] & (1 << (adrBit % 16)))
	{
		*pBit = 1;
	}
	else
...
uint8_t Modbus::SetRwBit(uint16_t adrBit,   uint8_t pBit)
{
	if (adrBit >= SIZE_RW_BITS) return ERROR_OF_ADR;
	if (pBit == 0) return ERROR_OF_POINTER;
	...
	if (pBit)
	{
		rw_bitArea[adrBit/16] |= (1 << (adrBit % 16));
	}
	else
	{
		rw_bitArea[adrBit/16] &= ~(1 << (adrBit % 16));
